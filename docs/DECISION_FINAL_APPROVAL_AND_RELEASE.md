...
| `POST_DECISION_UPDATE` | Directly consumes the approved Card and embeds the affirmative approval reference and exact `approved_negotiation_strategy_ids`. The approval subject must match the Card triplet, action, case, run, and exact action payload derived from the Card. Stable artifact identity keeps the approval's business substance while excluding request/run IDs and timestamps. Founder approval authorizes the negotiation direction; it is not evidence that the customer agreed or that the target margin was achieved. |
| `EXTERNAL_DOCUMENT_SUBMISSION_PROPOSAL` | Exists only for approved `ACCEPT` plus an exact package. Its three direct inputs are the post-decision update, Decision Card, and `DOCUMENT_RELEASE_PACKAGE`; its payload pins all three triplets plus package ID, recipient/purpose, document and masking-manifest item IDs, approval-condition codes, and evidence union. It contains references and release metadata, not the sanitized payload bytes. |
