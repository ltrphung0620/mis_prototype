...
| `contract_requirements` | Các requirement đã được Planner phân loại theo type/certainty, kèm exact Credit Profile amount khi quan hệ và dữ liệu đủ rõ. |
...
### 7.7 `ContractRequirement`

Planner tạo requirement từ exact configured source phrases và chỉ gắn Credit Profile khi đồng thời
khớp OPC company ID, request type và đúng một canonical contract-ID token. Similar name hoặc mô tả
tự do riêng lẻ không đủ để tạo quan hệ.

| Field | Ý nghĩa |
|---|---|
| `requirement_id` | ID deterministic của requirement và evidence nguồn. |
| `requirement_type` | `PERFORMANCE_BOND`, `WORKING_CAPITAL` hoặc `TRADE_FINANCE_LC`. |
| `certainty` | `REQUIRED` nếu nguồn khai báo bắt buộc; `POSSIBLE` nếu chỉ là khả năng. |
| `requested_amount` | Positive integer lấy đúng từ Credit Profile được liên kết; nullable khi type/certainty không yêu cầu Planner block. |
| `requested_amount_currency` | Canonical `VND`. |
| `amount_semantics` | Khi có amount, hiện là `CREDIT_PROFILE_REQUESTED_AMOUNT`. |
| `credit_case_id` | Exact Credit Profile record đã liên kết; có thể vẫn hiện diện khi amount của record thiếu hoặc không hợp lệ. |
| `source_record_ids` / `source_fields` | Các field contract/order khai báo requirement. |
| `evidence_ids` | Evidence closure của requirement, quan hệ và raw `10_CREDIT_PROFILE.requested_amount`. |

Với `PERFORMANCE_BOND + REQUIRED`, thiếu một quan hệ Credit Profile duy nhất hoặc thiếu positive
integral amount là Planner blocker. Planner không chuyển việc nhập amount cho Founder. Amount này là
số tiền được yêu cầu/tham chiếu trong Credit Profile, không phải số tiền ngân hàng hỗ trợ, chấp thuận
hay đã phát hành bảo lãnh.
...
| `USER_INPUT` | Evidence do người dùng cung cấp cho các gap thực sự; có thể gồm legacy `BANKING_INPUT_SUPPLEMENT`, nhưng normal performance-bond amount dùng `TEAM_PACK` evidence. |
...
| Giá trị | Ý nghĩa |
|---|---|
| `BLOCKING` | Thiếu dữ liệu làm workflow phải dừng ở node sở hữu requirement, ví dụ Planner Intake hoặc post-precheck evidence review. |

Planner tạo missing-data request cho base-data blockers, bao gồm required performance bond không có
một exact Credit Profile positive amount. Evidence gap không blocking vẫn là warning/limitation,
không phải `MissingDataRequest`.
...
Planner hoặc Decision tạo request ở trạng thái `OPEN`. `RESOLVED` chỉ được dùng sau khi server đã
validate đúng case, workflow, pending request và evidence. Legacy Banking amount supplement vẫn có
thể resolve artifact cũ, nhưng không thuộc normal flow của required performance bond.
...
Master Workflow hiện dùng `WAITING_FOR_INPUT` tại Planner và các evidence/document gaps thực sự.
...
| `BANKING_DISCOVERY_REQUEST` | Request nội bộ bất biến từ Decision; normal required performance-bond route mang exact Planner/Credit Profile requested amount và lineage. |
| `BANKING_OPTION_MATRIX` | Matrix facts/criteria deterministic; identity/version phụ thuộc exact request, amount lineage và catalog policy. |
...
| `BANKING_INPUT_SUPPLEMENT` | Artifact compatibility/recovery cho legacy null-amount request; không thuộc normal required performance-bond flow và không được override authoritative request amount. |
...
Trong normal required performance-bond flow, matrix đầu tiên nhận amount từ
`BANKING_DISCOVERY_REQUEST` và ghi `PASS` hoặc `FAIL` cho `MINIMUM_AMOUNT`. `NOT_EVALUABLE` vẫn là
enum hợp lệ cho dữ liệu legacy/không đủ để so sánh, nhưng required performance-bond amount thiếu sẽ
block ở Planner thay vì chờ Founder. Các monetary field không có cột currency riêng được chuẩn hóa
theo quy ước hệ thống là `VND`.

### 13.3 BankingDataGapCode

| Giá trị | Ý nghĩa |
|---|---|
| `REQUESTED_AMOUNT_UNAVAILABLE` | Legacy/unsupported request chưa có amount; normal required performance-bond case bị Planner block trước Banking. |
...
`REQUESTED_AMOUNT_UNAVAILABLE` được giữ cho backward compatibility, không mô tả normal flow mới.
Thiếu explicit Credit Profile cho một required performance bond là Planner blocker. API field
`company_profile` vẫn được map riêng tới `02_OPC_PROFILE`; Credit Profile amount không thay thế
company profile.
...
| `BANKING_DISCOVERY_REQUEST` | `amount` lấy từ request đã validate, với exact Planner requirement/Credit Profile lineage. |
| `BANKING_INPUT_SUPPLEMENT` | Legacy-only source cho artifact cũ; không được override non-null request amount. |
...
| `MISSING_INPUT` | Legacy mapped input chưa có; không phải normal required performance-bond outcome. |
...
| `INPUT_REQUIRED` | Legacy mapped input còn thiếu; normal required performance-bond amount được Planner resolve hoặc block trước Banking. |
...
| `BANKING_INPUT_REQUIRED` | Compatibility outcome cho legacy request thiếu mapped input; không phải normal required performance-bond route. |
...
### 13.10 BankingDiscoveryRequest amount fields

| Field | Ý nghĩa |
|---|---|
| `requirement_id` | Exact Planner contract requirement kích hoạt Banking route. |
| `credit_case_id` | Exact `10_CREDIT_PROFILE` record cung cấp amount. |
| `requested_amount` | Strict positive integer được carry, không được Decision/Banking tính lại. |
| `requested_amount_currency` | `VND`. |
| `amount_semantics` | `CREDIT_PROFILE_REQUESTED_AMOUNT`. |
| `amount_evidence_ids` | Đúng raw TeamPack evidence cho `10_CREDIT_PROFILE.requested_amount`. |
| `evidence_ids` | Toàn bộ requirement/relationship evidence closure. |

Các field binding phải cùng có hoặc cùng không có; normal required performance-bond request yêu cầu
đầy đủ. Evidence Validator và Banking context reject mọi amount không khớp
`EVALUATION_CASE.contract_requirements`. Đây là requested/reference amount, không phải
`supported_amount` hoặc bank-approved amount.

### 13.11 BankingInputSupplement fields — legacy compatibility

Artifact này không thuộc normal flow mới và không phải một Founder checkpoint. Nó chỉ hỗ trợ
recovery cho persisted request cũ chưa có authoritative amount.
...
Supplement không được thay đổi `BANKING_DISCOVERY_REQUEST`; nếu request đã có authoritative amount,
giá trị xung đột bị từ chối. `ArtifactStatus` vẫn là `CREATED`; artifact cũ không cần đổi sang
`STALE` hoặc `SUPERSEDED` để giữ audit history.

## 14. Normal Banking route đọc nhanh

Planner đã resolve requirement và amount trước khi route:
...
Founder không nhập `requested_amount`. Founder chỉ quyết định khi Governance tạo request cho exact
protected action/option theo policy đã load. Số tiền được carry vào Governance vẫn là requested
Credit Profile amount, không tự trở thành supported/approved amount.
...
Internal Decision Package chỉ tổng hợp evidence và cũng không phải approval subject. Decision phase
đã implement sẽ tạo deterministic scenario, guarded `AI_DECISION_ANALYSIS` và exact
`DECISION_CARD`; các draft này đều qua Evidence Validator trước persistence. Chỉ một
`EXTERNAL_DOCUMENT_SUBMISSION_PROPOSAL` được tạo sau khi Founder approve exact `ACCEPT` Card có
package mới kích hoạt checkpoint gửi tài liệu. Proposal phải qua Evidence Validator và một
Governance approval request riêng. Workflow dừng tại `READY_FOR_EXTERNAL_SUBMISSION`; connector,
external delivery và provider receipt chưa được implement.
