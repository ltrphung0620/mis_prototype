...
from opc_mis.domain.post_decision_models import PostDecisionOutcome, PostDecisionUpdate
...
_STATUS_LABELS = {
    ...
    DashboardTaskStatus.WAITING_FOR_DEMO: "Đang tạm dừng để demo",
...
_EXECUTION_LABELS = {
    ...
    WorkflowStatus.WAITING_FOR_DEMO: "Đang tạm dừng để demo",
...
_BUSINESS_LABELS = {
    ...
    DashboardBusinessStatus.DEMO_PAUSED: "Đang tạm dừng để trình bày demo",
...
_RECOMMENDATION_LABELS = {
    ...
    DecisionRecommendation.NEGOTIATE_CONDITIONS_TO_ACCEPT: "Chấp nhận có điều kiện",
    DecisionRecommendation.DO_NOT_ACCEPT: "REJECT Â· Từ chối",
...
_CURRENT_STAGE_LABELS = {
    ...
    WorkflowNode.NEGOTIATION_OUTCOME_RECEIVED.value: "Gửi điều kiện đàm phán",
    WorkflowNode.NEGOTIATION_FINAL_CONFIRMATION.value: "Gửi điều kiện đàm phán",
...
def _task_applicability(
    ...
    if task_id == "NEGOTIATION_TERMS_SENT":
        recommendation = context.summary.decision_recommendation
...
def _current_stage_status(
    ...
    mapping = {
        ...
        WorkflowStatus.WAITING_FOR_DEMO: DashboardTaskStatus.WAITING_FOR_DEMO,
...
def _stage_status(tasks: tuple[DashboardTask, ...]) -> DashboardTaskStatus:
    ...
    priorities = (
        ...
        DashboardTaskStatus.WAITING_FOR_DEMO,
        DashboardTaskStatus.WAITING_FOR_DEPENDENCIES,
...
def _business_status(summary: WorkflowRunSummary) -> DashboardBusinessStatus:
    ...
    if summary.status is WorkflowStatus.WAITING_FOR_DEMO:
        return DashboardBusinessStatus.DEMO_PAUSED
...
def _external_release_required(
    *,
    ...
    if update.external_document_release_required:
        return True
    terms_node = nodes.get(WorkflowNode.NEGOTIATION_TERMS_SENT)
    return (
        update.outcome is PostDecisionOutcome.NEGOTIATION_AUTHORIZED
        and update.document_release_package is not None
        and terms_node is not None
        and terms_node.status is WorkflowNodeStatus.COMPLETED
    )
