...
class PlannerRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        workflow_step_delay_seconds: float | None = None,
    ) -> None:
        self.workbook_path = workbook_path.resolve()
        self.dataset_id = dataset_id
        resolved_settings = settings or AppSettings.from_environment()
        resolved_workflow_step_delay_seconds = (
            workflow_step_delay_seconds
            if workflow_step_delay_seconds is not None
            else resolved_settings.workflow_step_delay_seconds
        ...
        self._case_workflow_orchestrator = CaseWorkflowOrchestrator(
            ...
            approvals=self._approval_requests,
            events=self._runtime_events,
            workflow_step_delay_seconds=resolved_workflow_step_delay_seconds,
        )
    ...
    async def demo_pause_case_workflow(
        self, workflow_run_id: str, reason: str
    ) -> WorkflowStartResult:
        """Pause a running automatic workflow for live presentation control."""
        run = await self._case_workflow_orchestrator.demo_pause(
    ...
    async def demo_resume_case_workflow(self, workflow_run_id: str) -> WorkflowStartResult:
        """Continue a workflow paused only for a live demo."""
        run = await self._case_workflow_orchestrator.demo_resume(workflow_run_id)
        if self._runner_started:
            await self._workflow_runner.enqueue(workflow_run_id)
        else:
            await self._case_workflow_orchestrator.execute(workflow_run_id)
