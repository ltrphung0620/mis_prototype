...
@router.post(
    "/workflows/{workflow_run_id}/demo-pause",
    response_model=WorkflowStartResult,
    status_code=status.HTTP_202_ACCEPTED,
    tags=["Workflow"],
    summary="Pause a running workflow for a live demo",
)
async def demo_pause_case_workflow(
    workflow_run_id: str,
    payload: DemoPauseRequest,
    request: Request,
) -> WorkflowStartResult:
    """Pause automatic progression without creating governance approval evidence."""
    try:
        return await _runtime(request).demo_pause_case_workflow(
            workflow_run_id, payload.reason
        )
    except CaseWorkflowNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except CaseWorkflowConflictError as exc:
...
@router.post(
    "/workflows/{workflow_run_id}/demo-resume",
    response_model=WorkflowStartResult,
    status_code=status.HTTP_202_ACCEPTED,
    tags=["Workflow"],
    summary="Resume a workflow paused only for a live demo",
)
async def demo_resume_case_workflow(
    workflow_run_id: str,
    request: Request,
) -> WorkflowStartResult:
    """Continue after a manual presentation pause."""
    try:
        return await _runtime(request).demo_resume_case_workflow(workflow_run_id)
    except CaseWorkflowNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except CaseWorkflowConflictError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
