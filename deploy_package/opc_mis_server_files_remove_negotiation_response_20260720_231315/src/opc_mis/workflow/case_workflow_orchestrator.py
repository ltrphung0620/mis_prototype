    ...
    async def confirm_negotiation_terms_sent(
        ...
        *,
        ...
        updated = await self._save_run(
            ...
            current_stage=WorkflowNode.NEGOTIATION_IN_PROGRESS.value,
            pending_request_ids=(),
            resume_stage=None,
            failure_reason=None,
        )
        ...
        events = await self._events.list_after(updated.workflow_run_id, 0)
        if not any(item.event_type == "WORKFLOW_COMPLETED" for item in events):
            await self._event(updated, "WORKFLOW_COMPLETED", None)
        return updated
