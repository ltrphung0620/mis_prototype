...
class ExternalDocumentSubmissionProposalBuilder:
    ...
    def build(
        ...
        accepted_route = (
            update.outcome is PostDecisionOutcome.FINAL_DECISION_ACCEPTED
            and update.external_document_release_required
        )
        negotiated_terms_sent_route = (
            update.outcome is PostDecisionOutcome.NEGOTIATION_AUTHORIZED
            and update.recommendation
            is DecisionRecommendation.NEGOTIATE_CONDITIONS_TO_ACCEPT
        )
        if (
            not (accepted_route or negotiated_terms_sent_route)
        ...
        effective_execution_status = (
            update.contract_execution_status
            if accepted_route
            else ContractExecutionStatus.SIGNED
        )
        if effective_execution_status is not ContractExecutionStatus.SIGNED:
        ...
        proposal_id = external_document_submission_proposal_id(
            ...
            contract_execution_status=effective_execution_status,
