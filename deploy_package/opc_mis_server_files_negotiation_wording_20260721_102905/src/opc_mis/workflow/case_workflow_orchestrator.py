...
class CaseWorkflowOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self._workflow_step_delay_seconds = max(0.0, workflow_step_delay_seconds)
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        return
                    await self._raise_if_demo_paused(run.workflow_run_id)
    ...
    async def _finish_node(
        ...
        *,
        ...
        honor_step_pacing: bool = True,
    ) -> None:
        await self._raise_if_demo_paused(node.workflow_run_id)
        artifact_ids = tuple(
            dict.fromkeys(
        ...
        if honor_step_pacing:
            await self._pace_after_completed_node(completed)
    ...
    async def _save_run(
        ...
        *,
        ...
        if (
            ...
            and status is not WorkflowStatus.WAITING_FOR_DEMO
        ):
    ...
    async def _raise_if_demo_paused(self, workflow_run_id: str) -> None:
        latest = await self._workflows.get_run(workflow_run_id)
        if latest is not None and latest.status is WorkflowStatus.WAITING_FOR_DEMO:
            raise _DemoPauseInterrupted

    async def _pace_after_completed_node(self, node: WorkflowNodeState) -> None:
        if (
            self._workflow_step_delay_seconds <= 0
            or not self._node_completed(node)
            or node.node in _OPENAI_THINKING_NODES
        ):
            return
        await self._raise_if_demo_paused(node.workflow_run_id)
        await self._sleeper(self._workflow_step_delay_seconds)
        await self._raise_if_demo_paused(node.workflow_run_id)
